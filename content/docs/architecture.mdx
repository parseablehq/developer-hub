...
This document is organized into specific sections for each sub-system like ingestion, query, search, and index. To understand the specific decisions and trade-offs, refer the [design choices document](/design-choices).
...
![Parseable Ingestion Architecture](./static/ingest-arch.webp)

## Query

<Callout type="info">
  Distributed Query nodes are Enterprise only features available in cloud and enterprise versions and cannot be deployed with OSS versions.
</Callout>
...
![Parseable Query Architecture](./static/query-arch.webp)
...
There is no separate consensus layer, eliminating complex coordination and reducing operational overhead. Object storage manages all concurrency control. See [AWS S3 configuration](/storage/awss3) for detailed setup instructions.
