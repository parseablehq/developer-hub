...
For log data you can use the HTTP output plugins of all the logging agents/shippers like [Fluent Bit](/docs/datasource/log-agents/fluent-bit), [Vector](/docs/datasource/log-agents/vector), [syslog-ng](/docs/datasource/log-agents/syslog), [LogStash](/docs/datasource/log-agents/logstash), among others to send log events to Parseable.
...
<Card href="/docs/ingest-data/cloud/azure-api-service" icon={<IconBrandAzure className="text-blue-600" />} title='Azure API Service'>
Ingest streaming data from Azure API Service.
</Card>
