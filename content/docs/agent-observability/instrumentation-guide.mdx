...
redirect_from:
  - /agent-observability/instrumentation-guide
---

How to instrument any GenAI agent application to emit OpenTelemetry traces and correlated logs for full observability — model metadata, token usage, latency, complete conversation content, tool calls, and reasoning blocks.
...
You need two tracers — one in the agent orchestration module (for `invoke_agent` and `execute_tool` spans), one in the LLM call module (for `chat` spans and log records):
...
**Multiple log records** (in the logs pipeline), all carrying matching `trace_id` + `span_id` from `chat` spans:
...
No truncation. No size limits from the instrumentation side. The body carries whatever the LLM returned or whatever was sent to it.
...
| **All event types present** | Check `event_name` values across log records | `gen_ai.system.message`, `gen_ai.user.message`, `gen_ai.choice`, and optionally `gen_ai.tool.call`, `gen_ai.thinking` |
