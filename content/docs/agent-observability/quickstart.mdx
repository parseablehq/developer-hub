...
title: Quickstart
redirect_from:
  - /agent-observability/quickstart
---

This guide covers three instrumentation paths to get GenAI traces into Parseable. Choose the one that fits your stack, then configure either an OTel Collector or direct export.
...
Once your application is running and making LLM calls, verify that data is arriving in Parseable:
...
If `span_count` is greater than zero, your instrumentation is working. Proceed to the [Schema Reference](/docs/agent-observability/schema-reference) to understand the columns available, or jump to [SQL Query Templates](/docs/agent-observability/sql-queries) for ready-to-use analytics queries.
