...
| `P_MULTI_TENANCY` | `No` | Enable tenant-aware request handling and tenant-scoped storage layout. See [Multi-tenancy](/user-guide/multi-tenancy). | `false` | `true` |
...
| `P_HOT_TIER_DIR` | `No` | Path on the query node (or standalone node), where the Parseable server would store recent data. Refer [Hot Tier documentation](/user-guide/smart-cache) for details. | `-` | `/home/user/hot-tier` |
| `P_MAX_DISK_USAGE_PERCENT` | `No` | Maximum percentage of total disk that should be used for hot tier. Refer [Hot Tier documentation](/user-guide/smart-cache) for details. | `80` | `70` |
...
Refer to [OIDC](/user-guide/openid) section for more details.
