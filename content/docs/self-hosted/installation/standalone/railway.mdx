...
Railway provisions the Bucket, attaches a persistent volume at `/data/staging`, injects the Bucket credentials into the Parseable service as variable references, builds the container from `parseable/parseable:edge`, and starts the server with the `parseable s3-store` command.
