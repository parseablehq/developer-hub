...
- `staging.enabled: true` creates a staging PVC. `false` uses temporary `emptyDir` storage.
- With `local-store`, `data.enabled: true` creates a data PVC. `false` uses temporary `emptyDir` storage.
