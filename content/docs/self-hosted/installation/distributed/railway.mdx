...
For the simpler single-node deployment, see the [standalone Railway template](/self-hosted/installation/standalone/railway).
...
You can add additional environment variables from the Railway dashboard as needed. You can find the details of all the environment variables in the [Environment Variables](/self-hosted/configuration) section.
...
For autoscaled ingestion or larger clusters, run Parseable on Kubernetes with the [OSS Helm chart](/self-hosted/installation/distributed/k8s-helm-oss) instead.
