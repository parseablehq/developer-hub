...
Railway provisions the Bucket, attaches a persistent volume at `/data/staging` on each node, injects the Bucket credentials into both services as variable references, builds the container from `quay.io/parseablehq/parseable:edge`, and starts each node with the `parseable s3-store` command. The query node starts in `P_MODE=query` and the ingest node in `P_MODE=ingest`.
...
For autoscaled ingestion or larger clusters, run Parseable on Kubernetes with the [OSS Helm chart](/docs/self-hosted/installation/distributed/k8s-helm-oss) instead.
