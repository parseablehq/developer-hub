...
A source here can be anything that generates observability data, i.e. agents like FluentBit, FluentD, Vector, agents from the OTel ecosystem or the application itself.
...
However, we recommend ideally keeping sources with similar schema in a single dataset, and sources with different schemas in separate datasets. This allows lesser variations in the schema, which in turn allows for better compression and faster query performance.

Let's take a look at some of the common scenarios:

- **Kubernetes infrastructure logs**: Kubernetes infrastructure logs (e.g. kubelet, kube-proxy, etc.) can be mapped to a single dataset. This allows you to query the logs across all the Kubernetes components easily. Since these logs have similar schema, 
