...
`X-API-Key` authorizes the write to Parseable. `X-P-Stream` selects the Parseable dataset. Parseable treats the payload as metrics because Prometheus remote write sends it to `/v1/prometheus/write`. Change the `node` label when monitoring more than one Docker host.
