...
This guide configures Parseable to consume JSON records from Google Cloud Managed Service for Apache Kafka. It covers Parseable running in Google Kubernetes Engine (GKE) or on a Compute Engine VM.
...
The local authentication server and Parseable must share the same network namespace:

- On GKE, run the server as a sidecar in each ingestor pod.
- On Compute Engine, run the server on the same VM before starting Parseable.
