...
This guide configures Parseable ingestors running in Amazon EKS to consume JSON records from Amazon Managed Streaming for Apache Kafka (Amazon MSK). Authentication uses AWS IAM through EKS Pod Identity, so no long-lived AWS access keys, Kafka passwords, or OAuth client secrets are stored in Kubernetes.
...
Two independent paths are involved:

- **Data path:** Parseable connects privately to the MSK IAM bootstrap brokers on TCP `9098`, joins a Kafka consumer group, and consumes topic partitions.
- **Identity path:** The EKS Pod Identity Agent supplies temporary role credentials to the ingestor. Parseable's MSK signer uses those credentials and the AWS region to generate a short-lived OAuth bearer token. MSK validates the token and authorizes Kafka actions through IAM.
...
Poll the operation until it returns `UPDATE_COMPLETE`:
...
The result is a comma-separated broker list using port `9098`. Set this exact value as `P_KAFKA_BOOTSTRAP_SERVERS`; do not use the plaintext `9092` or TLS-only `9094` bootstrap address.
...
The Parseable pods must resolve the private MSK broker hostnames and reach every broker on TCP `9098`.

For clusters in the same VPC:

1. Allow inbound TCP `9098` on the MSK security group from the EKS node security group. If you use security groups for pods, use the ingestor pod security group instead.
2. Confirm the EKS subnets can route to the MSK broker subnets.
3. Ensure network ACLs permit the request and return traffic.

For different VPCs or accounts, configure VPC peering, Transit Gateway, or [MSK multi-VPC private connectivity](https://docs.aws.amazon.com/msk/latest/developerguide/aws-access-mult-vpc.html). Cross-account deployments also require the corresponding MSK cluster policy and IAM trust configuration.
...
EKS Pod Identity requires the agent on every worker node. EKS Auto Mode includes this capability; other EKS clusters need the add-on.
...
The worker-node role must include `eks-auth:AssumeRoleForPodIdentity`. The AWS-managed `AmazonEKSWorkerNodePolicy` includes this permission. Nodes in private subnets must also reach the EKS Auth API, either through outbound connectivity or an EKS Auth VPC interface endpoint.
...
The ingestor needs data-plane permissions for one cluster, its subscribed topics, and its consumer group. It does not need MSK administrative permissions or producer permissions.
...
The pod should remain ready without authentication or authorization errors. Publish at least one valid JSON record with a separately authorized producer:
...
The Parseable stream is created lazily after the first record is consumed and the batch is flushed. List streams:

```bash
curl --user "$PARSEABLE_USERNAME:$PARSEABLE_PASSWORD" \
