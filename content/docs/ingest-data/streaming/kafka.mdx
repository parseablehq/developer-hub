...
<Callout type="info">
For provider-specific managed Kafka setup, follow the [Amazon MSK on EKS guide](/ingest-data/streaming/aws-msk) or the [Google Cloud Managed Kafka guide](/ingest-data/streaming/gcp-managed-kafka).
</Callout>
...
- Amazon MSK IAM: `--oauth-provider=aws-msk` and `--aws-region=<region>`. AWS credentials are resolved through the AWS SDK default credential chain.
...
#### Metrics
All metrics listed in [librdkafka's statistics documentation](https://github.com/confluentinc/librdkafka/blob/master/STATISTICS.md) are available at the /metrics endpoint.
