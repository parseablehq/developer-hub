...
variables:
  ...
  OTEL_EXPORTER_OTLP_HEADERS: "Authorization=Bearer ${PARSEABLE_API_KEY},X-P-Stream=gitlab-traces"
...
| `PARSEABLE_API_KEY` | Variable (masked) | Parseable API key for OTLP tracing |
...
- Set up [alerts](/user-guide/alerting) for failed pipelines
- Create [dashboards](/user-guide/dashboards) for CI/CD metrics
- Configure [GitHub Actions](/ingest-data/cicd/github-actions) for multi-platform CI
