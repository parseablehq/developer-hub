...
env:
  ...
  OTEL_EXPORTER_OTLP_HEADERS: "Authorization=Bearer ${{ secrets.PARSEABLE_API_KEY }},X-P-Stream=github-actions-traces"
...
| `PARSEABLE_API_KEY` | Parseable API key for OTLP tracing |
...
- Set up [alerts](/user-guide/alerting) for failed builds
- Create [dashboards](/user-guide/dashboards) for CI/CD metrics
- Configure [Jenkins](/ingest-data/cicd/jenkins) for additional pipelines
