...
The Bearer token authenticates the request with Parseable. `X-P-Stream` tells Parseable which dataset should receive the traces. Parseable identifies the signal as traces from the `/v1/traces` endpoint.
...
OpenRouter uses `Authorization: Bearer <OPENROUTER_API_KEY>` for its own API. Parseable uses `Authorization: Bearer <parseable-api-key>` on the OTLP destination or inside the Collector exporter.
...
- If traces reach the Collector but not Parseable, check the Collector logs and confirm `Authorization` and `X-P-Stream` are present in the exporter headers.
