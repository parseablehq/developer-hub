...
`X-API-Key` authenticates the request with Parseable. `X-P-Stream` tells Parseable which dataset should receive the traces. Parseable identifies the signal as traces from the `/v1/traces` endpoint.
...
- If traces reach the Collector but not Parseable, check the Collector logs and confirm `X-API-Key` and `X-P-Stream` are present in the exporter headers.
