...
After traces are flowing, you can build a dashboard for request volume, token usage, cost, latency, and provider behavior. Public Parseable dashboard examples live in [parseablehq/dashboards](https://github.com/parseablehq/dashboards). Use them as a starting point, then update dataset names and queries for your `openrouter-traces` dataset.
