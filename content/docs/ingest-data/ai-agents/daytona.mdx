...
The OpenTelemetry Collector is recommended for sandbox telemetry because Daytona uses one organization-level OTLP configuration. Each Parseable dataset needs its own `X-P-Stream` header. The Collector receives all signals from Daytona and routes each one to the correct Parseable dataset.
...
- A Parseable Cloud or Enterprise instance reachable from your application and from the OpenTelemetry Collector. These examples use OTLP over HTTP with Protocol Buffers.
- A Parseable API key with ingest access, sent as a Bearer token
- Python 3 if you want to run the SDK example below
- An OpenTelemetry Collector for routing sandbox logs, traces, and metrics

If your Parseable deployment still uses Basic Auth, replace the Bearer token with a Basic Auth value in the examples.
...
export OTEL_EXPORTER_OTLP_HEADERS="Authorization=Bearer <parseable-api-key>,X-P-Stream=daytona-sdk-traces"
```
...
When this is set up, the `daytona-logs` dataset gives you the runtime side of the sandbox run. You can inspect the log volume over time, filter by log category, and open the matching records to see phase changes, stdout checkpoints, and other sandbox output in one place.

![Daytona sandbox logs in Parseable](./static/daytona-logs.png)
...
The `daytona-metrics` dataset is useful when you want to see how a sandbox behaved over time instead of only checking whether it succeeded. In practice, this is where CPU bursts, memory spikes, and filesystem changes are easiest to spot during longer code runs.

![Daytona sandbox metrics in Parseable](./static/daytona-metrics.png)
...
For sandbox telemetry, use the Collector. Daytona's organization setting uses one endpoint and one header set for logs, traces, and metrics. The Collector lets you send each signal to the correct Parseable dataset with its own `X-P-Stream` header.
...
- If authentication fails, confirm the API key has ingest access and is sent as `Authorization: Bearer <parseable-api-key>`.
...
- If only one signal appears, check that the Collector has separate logs, traces, and metrics pipelines, and that each exporter uses the intended `X-P-Stream`.
- If metrics are delayed, wait at least 60 to 90 seconds after the workload starts.

For more protocol details, see [Daytona OpenTelemetry collection](https://www.daytona.io/docs/en/observability/otel-collection/), [Parseable OTLP logs](/ingest-data/otel/logs), [Parseable OTLP traces](/ingest-data/otel/traces), and [Parseable OTLP metrics](/ingest-data/otel/metrics).
