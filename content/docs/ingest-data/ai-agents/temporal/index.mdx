---
title: Temporal
description: Observe Temporal workers and Temporal Cloud in Parseable
---

Temporal is a durable execution platform for workflows and applications. Parseable can observe both sides of a Temporal deployment: workflow and activity telemetry emitted by your workers and service metrics exposed by Temporal Cloud.

<PageTabs items={['Worker telemetry', 'Temporal Cloud metrics']}>
  <PageTab value="Worker telemetry">

Use Parseable's Temporal middleware plugins to send workflow and activity logs and traces directly from your workers. Temporal SDK metrics can also be scraped from each worker and sent to Parseable with Prometheus Remote Write.

Pick your SDK below to install the plugin. Continue through this page for the shared event schema, queries, dashboards and operational caveats.
...
|-------------|----------------------------------------------------------------------|------------------------------|
| TypeScript  | [`@parseable/temporal`](https://www.npmjs.com/package/@parseable/temporal) | [TypeScript](/ingest-data/ai-agents/temporal/typescript) |
| Python      | [`temporal-parseable`](https://pypi.org/project/temporal-parseable/)      | [Python](/ingest-data/ai-agents/temporal/python)         |
| Java        | [`temporal-parseable`](https://central.sonatype.com/artifact/com.parseable/temporal-parseable) | [Java](/ingest-data/ai-agents/temporal/java)             |
...
![Temporal workflow logs in Parseable](./static/temporal-logs.png)
...
![Temporal workflow traces in Parseable](./static/temporal-traces.png)
...
![Temporal SDK metrics in Parseable](./static/temporal-metrics.png)
...
After logs, traces and metrics are flowing, you can use dashboards to keep the main workflow signals in one place. Parseable provides a Temporal workflow observability dashboard in [parseablehq/dashboards](https://github.com/parseablehq/dashboards/tree/main/temporal-workflow-observability). Use it as a starting point, then update dataset names and queries for your Temporal deployment.

The spans dashboard helps you look at workflow and activity execution across runs without opening each trace manually.

![Temporal spans dashboard in Parseable](./static/temporal-span-dashboard.png)
...
![Temporal metrics dashboard in Parseable](./static/temporal-metrics-dashboard.png)
...
{/* End worker telemetry guide. */}

  </PageTab>
  <PageTab value="Temporal Cloud metrics">

Temporal Cloud publishes account and namespace metrics through an OpenMetrics endpoint. Run an OpenTelemetry Collector to scrape that endpoint and export the metrics to Parseable over OTLP/HTTP:

```text
Temporal Cloud /v1/metrics
  |
  | OpenMetrics scrape
  v
...
Create a Temporal Cloud service account, grant it the **Metrics Read-Only** role and generate an API key. Create a Parseable API key with ingest access to your metrics dataset. You also need a place to run the OpenTelemetry Collector, such as a Kubernetes cluster, container platform or VM.

In the examples below, `<metrics-dataset>` represents the Parseable dataset where you want to store Temporal Cloud metrics. Replace it with your dataset name.
    ...
    config:
      scrape_configs:
        - job_name: temporal-cloud
          ...
          authorization:
            type: Bearer
            credentials: "${env:TEMPORAL_CLOUD_API_KEY}"
          static_configs:
...
exporters:
  otlphttp/parseable:
    ...
    headers:
      ...
      X-P-Stream: "<metrics-dataset>"

service:
  pipelines:
    metrics/temporal-cloud:
      receivers: [prometheus/temporal-cloud]
      processors: [batch]
      exporters: [otlphttp/parseable]
```

Set `TEMPORAL_CLOUD_API_KEY` and `PARSEABLE_API_KEY`, replace `<parseable-host>`, then start the OpenTelemetry Collector with this configuration. The OTLP/HTTP exporter appends `/v1/metrics` to the Parseable endpoint.

<Callout type="info">
The Collector handles both the OpenMetrics scrape and the OTLP export. You do not need a Prometheus server for this path.
</Callout>

## Verify ingestion

After two scrape intervals, open **Metrics** in Parseable and select the dataset you configured as `<metrics-dataset>`. Look for series with the `temporal_cloud_v1_` prefix.

If the dataset remains empty, check the Collector logs first. A `401` from `metrics.temporal.io` points to the Temporal service account or API key. An export error points to the Parseable URL, API key or dataset permission.

## Links

- [Temporal Cloud metrics integrations](https://docs.temporal.io/cloud/metrics/openmetrics/metrics-integrations)
- [Parseable OpenTelemetry metrics ingestion](/ingest-data/otel/metrics)

{/* End Temporal Cloud metrics guide. */}
