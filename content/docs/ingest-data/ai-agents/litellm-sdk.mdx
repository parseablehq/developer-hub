...
- **Traces appear, but logs do not**
  ...
  Confirm `enable_events=True` in `OpenTelemetryConfig`, and check that the Collector has a `logs` pipeline exporting to `litellm-sdk-logs`.

- **Metrics do not appear**

  Confirm `enable_metrics=True` and check that the Collector has a `metrics` pipeline exporting to `litellm-sdk-metrics`.
