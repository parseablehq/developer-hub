...
For the standalone gateway or proxy setup, see [LiteLLM Gateway](/docs/ingest-data/gateways/litellm).
...
Use [LiteLLM Gateway](/docs/ingest-data/gateways/litellm) when traffic goes through the LiteLLM Proxy or Gateway. The gateway path adds proxy-level visibility such as authentication, routing, budgets, rate limits, Redis, PostgreSQL, and Prometheus `/metrics` scraping.
