---
title: LLM Observability
description: Monitor and debug LLM applications with Parseable
---

import { IconBrandOpenai, IconRobot, IconLink, IconBook, IconUsers, IconBrain, IconCode, IconSettingsAutomation, IconServer, IconRoute, IconTimeline } from '@tabler/icons-react';

Monitor, debug, and optimize your LLM applications with Parseable. Track API calls, token usage, latency, and errors across all major LLM providers and frameworks.

## Why LLM Observability?

LLM applications present unique observability challenges:

- **Non-deterministic outputs** - Same input can produce different results
- **High costs** - Token usage directly impacts costs
- **Latency sensitivity** - Response times affect user experience
- **Complex chains** - Multi-step workflows are hard to debug
- **Prompt engineering** - Need visibility into prompt effectiveness

## Supported Integrations

<Cards>
  <Card 
    title="OpenAI" 
    href="/ingest-data/ai-agents/openai"
    icon={<IconBrandOpenai />}
  >
    GPT-4, GPT-3.5, and other OpenAI models
  </Card>
  <Card 
    title="Anthropic" 
    href="/ingest-data/ai-agents/anthropic"
    icon={<IconRobot />}
  >
    Claude and Claude Instant models
  </Card>
  <Card
    title="LiteLLM SDK"
    href="/ingest-data/ai-agents/litellm-sdk"
    icon={<IconCode />}
  >
    Multi-provider LLM traces, logs, and metrics using OpenTelemetry
  </Card>
  <Card
    title="Pydantic AI"
    href="/ingest-data/ai-agents/pydantic-ai"
    icon={<IconRobot />}
  >
    Agent, model, and tool-call traces using OpenTelemetry
  </Card>
  <Card 
    title="OpenRouter" 
    href="/ingest-data/ai-agents/openrouter"
    icon={<IconRoute />}
  >
    Zero-code LLM observability via Broadcast
  </Card>
  <Card 
    title="vLLM" 
    href="/ingest-data/ai-agents/vllm"
    icon={<IconServer />}
  >
    High-performance LLM inference serving
  </Card>
  <Card 
    title="LangChain" 
    href="/ingest-data/ai-agents/langchain"
    icon={<IconLink />}
  >
    LangChain framework integration
  </Card>
  <Card 
    title="LlamaIndex" 
    href="/ingest-data/ai-agents/llamaindex"
    icon={<IconBook />}
  >
    LlamaIndex RAG applications
  </Card>
  <Card 
    title="AutoGen" 
    href="/ingest-data/ai-agents/autogen"
    icon={<IconUsers />}
  >
    Microsoft AutoGen multi-agent systems
  </Card>
  <Card 
    title="CrewAI" 
    href="/ingest-data/ai-agents/crewai"
    icon={<IconBrain />}
  >
    CrewAI agent orchestration
  </Card>
  <Card
    title="Mastra"
    href="/docs/ingest-data/ai-agents/mastra"
    icon={<IconBrain />}
  >
    TypeScript agent logs, traces, metrics, tokens, tools, and cost
  </Card>
  <Card 
    title="DSPy" 
    href="/ingest-data/ai-agents/dspy"
    icon={<IconCode />}
  >
    DSPy programmatic prompting
  </Card>
  <Card 
    title="n8n" 
    href="/ingest-data/ai-agents/n8n"
    icon={<IconSettingsAutomation />}
  >
    n8n workflow automation
  </Card>
  <Card
    title="Daytona"
    href="/ingest-data/ai-agents/daytona"
    icon={<IconServer />}
  >
    SDK traces, sandbox logs, traces, and metrics
  </Card>
  <Card
    title="DBOS"
    href="/ingest-data/ai-agents/dbos"
    icon={<IconServer />}
  >
    DBOS workflows, logs, traces, and Conductor metrics
  </Card>
  <Card
    title="Temporal"
    href="/ingest-data/ai-agents/temporal"
    icon={<IconTimeline />}
  >
    Temporal workflows, activities, and agent orchestration
  </Card>
  <Card
    title="Modal"
    href="/ingest-data/ai-agents/modal"
    icon={<IconServer />}
  >
    Serverless Python logs, traces and metrics via OpenTelemetry Collector
  </Card>
  <Card
    title="Restate"
    href="/ingest-data/ai-agents/restate"
    icon={<IconTimeline />}
  >
    Durable execution workflow traces and service logs via OpenTelemetry Collector
  </Card>
</Cards>

## What to Monitor

### API Calls & Responses

Track every interaction with LLM providers:

- Request parameters (model, temperature, max_tokens)
- Full prompts and completions
- Response metadata and finish reasons

### Token Usage & Costs

Monitor consumption to control costs:

- Input and output tokens per request
- Cost calculations by model
- Usage trends over time

### Latency & Performance

Measure response times:

- Time to first token (TTFT)
- Total response time
- Streaming vs non-streaming performance

### Errors & Failures

Debug issues quickly:

- Rate limit errors
- API failures and retries
- Timeout tracking

## Guides & Cookbooks

<Cards>
  <Card
    title="Agentic Observability"
    href="/cookbook/agentic-observability"
    icon={<IconRobot />}
  >
    End-to-end guide for monitoring AI agents
  </Card>
  <Card
    title="Tool Calls Monitoring"
    href="/cookbook/tool-calls"
    icon={<IconCode />}
  >
    Track and debug LLM tool/function calls
  </Card>
  <Card
    title="Instrumentation Guide"
    href="/cookbook/instrumentation"
    icon={<IconSettingsAutomation />}
  >
    Add observability to your LLM applications
  </Card>
</Cards>
