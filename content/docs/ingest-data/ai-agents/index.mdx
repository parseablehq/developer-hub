---
title: AI Infrastructure
description: Send telemetry from agent frameworks, sandboxes, LLM runtimes, and gateways to Parseable
---

import {
  IconBrandOpenai,
  IconBrandGithub,
  IconBrain,
  IconCode,
  IconLink,
  IconRobot,
  IconRoute,
  IconServer,
  IconSettingsAutomation,
  IconTimeline,
  IconUsers,
} from '@tabler/icons-react';

AI systems are usually made of more than one model call. A request can move through an agent framework, a gateway, a sandbox, a workflow runtime, a hosted model API, or a self-hosted inference server before the user sees the final answer.

This section groups Parseable integrations by the part of the AI stack they observe. Use the framework guides when you want agent, model, and tool-call traces. Use the sandbox and runtime guides when you need visibility into execution environments and long-running workflows. Use the LLM and gateway guides when the main question is about model traffic, token usage, latency, and errors.

## Agent frameworks

Agent frameworks usually carry the highest-level application context. They can show which agent ran, which prompt or task started the run, which model was called, which tool was used, and where the run failed or slowed down.

<Cards>
  <Card
    title="Pydantic AI"
    href="/ingest-data/ai-agents/pydantic-ai"
    icon={<IconRobot />}
  >
    Capture agent, model, and tool-call telemetry from Pydantic AI applications.
  </Card>
  <Card
    title="Restate"
    href="/ingest-data/ai-agents/restate-openai-agents"
    icon={<IconRoute />}
  >
    Trace durable agent execution, model calls, tools, retries, and recovery.
  </Card>
  <Card
    title="CrewAI"
    href="/ingest-data/ai-agents/crewai"
    icon={<IconUsers />}
  >
    Observe CrewAI crews, agents, tasks, LLM calls, and errors.
  </Card>
  <Card
    title="n8n"
    href="/ingest-data/ai-agents/n8n"
    icon={<IconSettingsAutomation />}
  >
    Observe n8n workflow automation through traces and related telemetry.
  </Card>
  <Card
    title="Mastra"
    href="/ingest-data/ai-agents/mastra"
    icon={<IconBrain />}
  >
    Send Mastra agent logs, traces, and metrics to Parseable.
  </Card>
  <Card
    title="LangChain"
    href="/ingest-data/ai-agents/langchain"
    icon={<IconLink />}
  >
    Send LangChain application telemetry to Parseable.
  </Card>
  <Card
    title="LlamaIndex"
    href="/ingest-data/ai-agents/llamaindex"
    icon={<IconBrain />}
  >
    Observe LlamaIndex retrieval and generation workflows.
  </Card>
  <Card
    title="AutoGen"
    href="/ingest-data/ai-agents/autogen"
    icon={<IconUsers />}
  >
    Track multi-agent AutoGen activity and failures.
  </Card>
  <Card
    title="DSPy"
    href="/ingest-data/ai-agents/dspy"
    icon={<IconCode />}
  >
    Monitor DSPy programs and prompt optimization workflows.
  </Card>
</Cards>

## Coding assistants

Coding assistants emit agent-level telemetry from interactive development sessions. Use these guides to inspect model calls, token usage, tool execution, latency, errors, and session activity.

<Cards>
  <Card
    title="Claude Code"
    href="/ingest-data/ai-agents/coding-assistants/claude-code"
    icon={<IconRobot />}
  >
    Export Claude Code events, metrics and optional traces to Parseable.
  </Card>
  <Card
    title="GitHub Copilot"
    href="/ingest-data/ai-agents/coding-assistants/github-copilot"
    icon={<IconBrandGithub />}
  >
    Export GitHub Copilot agent traces to Parseable.
  </Card>
  <Card
    title="OpenAI Codex"
    href="/ingest-data/ai-agents/coding-assistants/codex"
    icon={<IconBrandOpenai />}
  >
    Export Codex logs, traces and metrics directly to Parseable.
  </Card>
</Cards>

## Sandboxes and runtimes

Sandboxes and execution runtimes are where agent work becomes real system activity. These guides focus on telemetry from code execution, durable workflows, serverless jobs, and workflow orchestration layers.

<Cards>
  <Card
    title="Temporal"
    href="/ingest-data/ai-agents/temporal"
    icon={<IconTimeline />}
  >
    Observe Temporal workflows, activities, SDK telemetry, and metrics.
  </Card>
  <Card
    title="Daytona"
    href="/ingest-data/ai-agents/daytona"
    icon={<IconServer />}
  >
    Capture SDK traces and sandbox logs, traces, and metrics.
  </Card>
  <Card
    title="Modal"
    href="/ingest-data/ai-agents/modal"
    icon={<IconServer />}
  >
    Send serverless Python logs, traces, and metrics through OpenTelemetry.
  </Card>
  <Card
    title="DBOS"
    href="/ingest-data/ai-agents/dbos"
    icon={<IconTimeline />}
  >
    Observe durable workflows with logs, traces, and Conductor metrics.
  </Card>
  <Card
    title="Restate"
    href="/ingest-data/ai-agents/restate"
    icon={<IconTimeline />}
  >
    Send durable execution workflow traces and service logs to Parseable.
  </Card>
</Cards>

## LLMs

LLM integrations help when you want to observe model-facing traffic directly. This can mean hosted provider APIs, SDK-level calls, or self-hosted inference runtimes such as vLLM.

<Cards>
  <Card
    title="LiteLLM SDK"
    href="/ingest-data/ai-agents/litellm-sdk"
    icon={<IconCode />}
  >
    Observe LiteLLM SDK usage from application code.
  </Card>
  <Card
    title="OpenAI"
    href="/ingest-data/ai-agents/openai"
    icon={<IconBrandOpenai />}
  >
    Log OpenAI API calls, responses, latency, and token usage.
  </Card>
  <Card
    title="Anthropic"
    href="/ingest-data/ai-agents/anthropic"
    icon={<IconRobot />}
  >
    Capture Claude API calls, responses, errors, and token usage.
  </Card>
  <Card
    title="vLLM"
    href="/ingest-data/ai-agents/vllm"
    icon={<IconServer />}
  >
    Monitor self-hosted vLLM inference serving.
  </Card>
</Cards>

## Gateways

Gateways sit between applications and model providers. They are useful when teams want one place to manage routing, keys, budgets, policies, fallbacks, and model-provider telemetry.

<Cards>
  <Card
    title="OpenRouter"
    href="/ingest-data/ai-agents/openrouter"
    icon={<IconRoute />}
  >
    Send OpenRouter traces and dashboard telemetry to Parseable.
  </Card>
  <Card
    title="LiteLLM Gateway"
    href="/ingest-data/gateways/litellm"
    icon={<IconRoute />}
  >
    Observe LiteLLM Gateway traces and Prometheus metrics.
  </Card>
  <Card
    title="Cloudflare AI Gateway"
    href="/ingest-data/gateways/cloudflare-ai-gateway"
    icon={<IconRoute />}
  >
    Observe Cloudflare AI Gateway traces, model usage, latency, cost, and errors.
  </Card>
</Cards>
