...
class ParseableMetricExporter extends BaseExporter {
  ...
  constructor() {
    ...
    const metricExporter = new OTLPMetricExporter({
      ...
      headers: headers('mastra-metrics'),
    });
...
If a dataset remains empty, check the application logs for OTLP errors and verify the endpoint, API key, and `X-P-Stream` header.
...
- [Send OpenTelemetry traces to Parseable](/ingest-data/otel/traces)
- [Send OpenTelemetry logs to Parseable](/ingest-data/otel/logs)
- [Send OpenTelemetry metrics to Parseable](/ingest-data/otel/metrics)
