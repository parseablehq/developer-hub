...
exporter = { otlp-http = { endpoint = "https://<your-parseable-host>:8000/v1/logs", protocol = "json", headers = { Authorization = "Bearer <parseable-api-key>", X-P-Stream = "codex-logs", Content-Type = "application/json" } } }

trace_exporter = { otlp-http = { endpoint = "https://<your-parseable-host>:8000/v1/traces", protocol = "json", headers = { Authorization = "Bearer <parseable-api-key>", X-P-Stream = "codex-traces", Content-Type = "application/json" } } }

metrics_exporter = { otlp-http = { endpoint = "https://<your-parseable-host>:8000/v1/metrics", protocol = "json", headers = { Authorization = "Bearer <parseable-api-key>", X-P-Stream = "codex-metrics", Content-Type = "application/json" } } }
```
...
For a self-hosted deployment that uses basic authentication, replace `Authorization = "Bearer <parseable-api-key>"` in each exporter with:
...
- **No datasets appear:** confirm Codex can reach the Parseable ingestor and verify each endpoint, API key, and `X-P-Stream` value.
