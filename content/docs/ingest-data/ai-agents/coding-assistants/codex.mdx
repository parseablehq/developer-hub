...
Codex exports OpenTelemetry logs, traces, and metrics from local coding sessions. Its native `[otel]` configuration sends each signal directly to a separate Parseable dataset, without an OpenTelemetry Collector.
...
This reports unsupported or misspelled `config.toml` fields for the installed Codex version.
