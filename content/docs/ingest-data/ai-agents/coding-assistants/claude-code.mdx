...
export OTEL_EXPORTER_OTLP_LOGS_HEADERS="Authorization=Bearer ${PARSEABLE_API_KEY},X-P-Stream=claudecode-logs"

export OTEL_EXPORTER_OTLP_METRICS_PROTOCOL=http/json
export OTEL_EXPORTER_OTLP_METRICS_ENDPOINT="${PARSEABLE_URL}/v1/metrics"
export OTEL_EXPORTER_OTLP_METRICS_HEADERS="Authorization=Bearer ${PARSEABLE_API_KEY},X-P-Stream=claudecode-metrics"

claude
```

For a self-hosted deployment that uses basic authentication, replace `Authorization=Bearer ${PARSEABLE_API_KEY}` in each header value with `Authorization=Basic <base64-encoded-username-and-password>`.
...
export OTEL_EXPORTER_OTLP_TRACES_HEADERS="Authorization=Bearer ${PARSEABLE_API_KEY},X-P-Stream=claudecode-traces"
```
...
```json
{
  "env": {
    ...
    "OTEL_EXPORTER_OTLP_LOGS_HEADERS": "Authorization=Bearer <parseable-api-key>,X-P-Stream=claudecode-logs",
    "OTEL_EXPORTER_OTLP_METRICS_PROTOCOL": "http/json",
    "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT": "https://<your-parseable-host>:8000/v1/metrics",
    "OTEL_EXPORTER_OTLP_METRICS_HEADERS": "Authorization=Bearer <parseable-api-key>,X-P-Stream=claudecode-metrics"
  }
}
```
...
- **No datasets appear:** confirm Claude Code can reach Parseable and that each signal has the correct endpoint and `X-P-Stream` value.
