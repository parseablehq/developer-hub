...
export OTEL_EXPORTER_OTLP_HEADERS="Authorization=Bearer ${PARSEABLE_API_KEY},X-P-Stream=copilot-traces"
...
For a self-hosted deployment that uses basic authentication, replace `Authorization=Bearer ${PARSEABLE_API_KEY}` with `Authorization=Basic <base64-encoded-username-and-password>` in `OTEL_EXPORTER_OTLP_HEADERS`.
...
- **Parseable rejects exports:** use the Parseable ingestor base URL and verify `X-P-Stream=copilot-traces`.
