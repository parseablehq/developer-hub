...
To query or ingest Parseable data inside a workflow, use the [Parseable community node for n8n](/docs/integrations/application-platforms/n8n).
...
exporters:
  otlphttp/parseabletraces:
    ...
    headers:
      Authorization: "Bearer <YOUR_PARSEABLE_API_KEY>"
...
If your Parseable deployment still uses basic authentication, replace the Bearer token with a Basic Auth value.
...
![n8n workflow traces in Parseable](./static/n8n-traces.png)
...
After traces are flowing, you can use a dashboard to keep the main workflow signals in one place instead of writing ad hoc queries each time. Parseable provides an n8n workflow observability dashboard in [parseablehq/dashboards](https://github.com/parseablehq/dashboards/tree/main/n8n-workflow-observability). Use it as a starting point, then map the dataset variable to your `n8n-traces` dataset.

The dashboard covers execution volume, success rate, average duration, the slowest nodes, node execution counts by type, a per-execution summary table and a recent errors table - all from the `workflow.execute` and `node.execute` spans described above.

![n8n workflow observability dashboard in Parseable](./static/n8n-dashboard.png)
...
- [Parseable OpenTelemetry traces](/ingest-data/otel/traces)
- [Parseable OpenTelemetry logs](/ingest-data/otel/logs)
