...
Use the exporter's **Authorization** field for the Bearer token. Do not also configure an API key header for the same Parseable endpoint.
...
- **Parseable returns an authentication error:** Verify the Bearer token in the exporter's Authorization field.
- **Parseable reports a missing stream:** Create `cf-ai-gateway-traces` and set `X-P-Stream` to that exact value.
