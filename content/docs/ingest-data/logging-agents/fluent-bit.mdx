...
- If Parseable returns `401` or `403`, check that `X-API-Key` is present and the key can write to the dataset.
- If Parseable returns `400`, confirm that the output uses JSON for `/api/v1/ingest`, or OTLP logs with `X-P-Log-Source: otel-logs` for `/v1/logs`.
