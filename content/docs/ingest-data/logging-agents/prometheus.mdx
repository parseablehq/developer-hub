...
The Bearer token authorizes the write to Parseable. `X-P-Stream` decides the target metrics dataset, and Parseable treats the payload as metrics data because it arrives on the remote-write endpoint.
...
- Confirm the exporter is sending to the metrics endpoint.
