...
redirect_from:
  - /OpenTelemetry/logs
  - /server/opentelemetry/opentelemetry-logs
  - /server/opentelemetry/logs
...
If the log body is a valid JSON string, Parseable automatically flattens its key-value pairs into separate fields in the ingested record. Each key is prefixed with `body_`. The original `body` field is also preserved as-is.
