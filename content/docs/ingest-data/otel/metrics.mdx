...
You can ingest OpenTelemetry metrics sent from the OTEL collectors in the JSON format to Parseable. In order to do so, use the default endpoint `/v1/metrics` at which the OTEL collector sends the metric data.

Here is a sample configuration of the OTEL collector to send the metrics to Parseable
...
By default Parseable infers timestamp data types for time related fields (e.g. `start_time_unix_nano`, `time_unix_nano`) so they can be queried as native timestamps. If you want to keep these fields as raw strings (`utf8`) instead, set the `x-p-infer-timestamp` header to `false` when creating the dataset.

```bash
curl -X PUT https://<parseable-endpoint>/api/v1/logstream/otel-metrics \
  -H "Authorization: Bearer <parseable-api-key>" \
...
- Is only valid for OTLP metrics datasets. Setting it for other telemetry types will return a `400 Bad Request`.
</Callout>
