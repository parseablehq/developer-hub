...
| `P_HOT_TIER_DIR` | `No` | Path on the query node (or standalone node), where the Parseable server would store recent data. Refer [Hot Tier documentation](/docs/features/smart-cache) for details. | `-` | `/home/user/hot-tier` |
| `P_MAX_DISK_USAGE_PERCENT` | `No` | Maximum percentage of total disk that should be used for hot tier. Refer [Hot Tier documentation](/docs/features/smart-cache) for details. | `80` | `70` |
