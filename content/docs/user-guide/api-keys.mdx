...
A Parseable instance has three types of identities — **Native** (Basic Auth), **OAuth** (OIDC), and **API Key**. API keys behave like users, i.e. they are assigned roles, and each role is a set of [RBAC](/user-guide/rbac) privileges. The key inherits exactly the access its roles grant.
...
Roles must already exist in the tenant/organization. See [RBAC](/user-guide/rbac) for how to create roles.
...
Because API keys reuse the role system, you can pick any role supported by Parseable RBAC. The role JSON is identical to what you would assign to a Native user — see [RBAC](/user-guide/rbac) for the full schema.
