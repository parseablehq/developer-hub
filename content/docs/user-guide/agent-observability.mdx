...
| [Hermes Agent](/ingest-data/ai-agents/hermes) | Agent invocations, token usage, model calls, tools, logs, and metrics | [OpenLIT](/ingest-data/ai-agents/openlit) | Agent, workflow, tool, and model telemetry |
...
Agent Observability is framework-aware when the incoming traces include agent, model, and tool attributes. Some frameworks emit those attributes directly. Others use instrumentation libraries such as OpenLIT or OpenInference to translate framework activity into OpenTelemetry spans.
