...
- **Event rows** have `event_name` set (e.g., `gen_ai.user.message`, `gen_ai.choice`, or `gen_ai.tool.output`). Parseable flattens event attributes with an `event_` prefix, so event content and event metadata stay queryable alongside the parent span.
...
| Column | Type | Description |
|--------|------|-------------|
| `event_name` | String | Event type such as `gen_ai.user.message`, `gen_ai.choice`, `gen_ai.tool.call`, `gen_ai.tool.input`, or `gen_ai.tool.output`. |
