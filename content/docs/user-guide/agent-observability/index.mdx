...
Once your agent is instrumented, the Agents page becomes the place to understand how your agents are behaving in production. You can see how often they run, how much they cost, where they spend time, what they call, and where the answer starts to drift.

<Callout type="idea">
Parseable enriches GenAI telemetry at ingest time with fields like `p_genai_cost_usd`, `p_genai_tokens_total`, `p_genai_tokens_per_sec`, and `p_genai_duration_ms`, so cost, token, and duration analysis is available without client-side tracking.
</Callout>
