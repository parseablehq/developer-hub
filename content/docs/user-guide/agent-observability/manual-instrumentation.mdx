...
This page is reference manual on how to manually instrument your agent with OTel spans and logs. If you are looking for a quick way to get LLM call traces flowing into Parseable, refer to the [Auto-instrumentation guide](/docs/user-guide/agent-observability/auto-instrumentation) which uses auto-instrumentation or two-line SDK initialization.
...
|------|--------------|-----|
| **Agent run** — which agent ran, total tokens, total cost, exit status | OTel **`invoke-agent` span** | End-to-end agent observability, run-level dashboards |
...
# Required: disable auto-instrumentors to prevent duplicate/noisy spans
...
| `OTEL_PYTHON_DISABLED_INSTRUMENTATIONS` | Comma-separated list of auto-instrumentors to skip. Disable model SDKs, HTTP clients, template engines, DB/framework instrumentors, and any other instrumentation that would create duplicate `POST`, `jinja2.compile`, or framework spans around your manual GenAI spans. | Yes (when using manual instrumentation) |
...
# Should include: openai,openai_v2,requests,urllib3,httpx,jinja2
```

If OpenAI, HTTP client, Jinja2, DB, or framework auto-instrumentors are installed and not disabled, they can create their own spans alongside your manual spans. Common unwanted child spans are HTTP `POST` and `jinja2.compile`.
