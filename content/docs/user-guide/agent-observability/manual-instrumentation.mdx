...
:::info
If duplicate spans still appear, check whether auto instrumentation is enabled in the runtime. For Python, review environment variables such as `OTEL_PYTHON_DISABLED_INSTRUMENTATIONS` and disable any framework, HTTP, database, template, or model-client instrumentation that is creating spans you do not need.
:::
