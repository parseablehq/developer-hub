...
When production is misbehaving, the Errors page is usually the first place to look. Instead of scrolling through the Traces explorer with a filter on error status, it collapses thousands of exception spans into a short list of distinct failures — one row per `(service, exception type, exception message)` — so you can see the *shape* of what is breaking before you start reading stack traces.

![Errors page overview with grouped errors and faceted filters](../static/errors-list.png)

The page works with **OTel Trace** datasets. Error groups are built from the `event_exception.type`, `event_exception.message`, and `event_exception.stacktrace` fields that OpenTelemetry records on span events. If you are chasing log-level errors instead, start from the [Logs explorer](/docs/user-guide/logs) with a filter on your log level field — log-based error grouping is not available here yet.

## When to use this page

Reach for the Errors page when:
...
- **Trace → logs** — from an error trace, jump to every log line that shares the same `trace.id` to see what the services in the request were logging around the failure
- **Span → logs** — from a specific error span, jump to log lines that share the same `span.id` for the narrow slice of logs tied to that one operation
...
### 6. Escalate to Keystone when the surface info is not enough
...
- The AI Summary is correct but not specific enough — you want to ask follow-ups like "are other services seeing the same upstream error right now?"
...
- **Always compare first seen and last seen before writing an RCA.** The same `14 UNAVAILABLE` that has been quietly failing all week is a very different story from one that started ten minutes ago.
