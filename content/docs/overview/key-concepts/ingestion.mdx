...
You can send Log events to Parseable via HTTP POST requests with data as JSON payload. You can use the HTTP output plugins of all the common logging agents like [FluentBit](/ingest-data/logging-agents/fluent-bit), [Vector](/ingest-data/logging-agents/vector), [syslog-ng](/ingest-data/logging-agents/syslog), [LogStash](/ingest-data/logging-agents/logstash), among others to send log events to Parseable.

You can also directly integrate Parseable with your application via the [ingestion API](/api/v1/ingestLogsWithHeaders).
