...
def export_to_siem(stream, start_time, end_time):
    # Query Parseable
    query = f"""
    SELECT * FROM "{stream}"
    WHERE p_timestamp >= '{start_time}'
      AND p_timestamp < '{end_time}'
    """
...
streams = ['security-logs', 'audit-logs', 'access-logs']
for stream in streams:
    ...
    print(f"Exported {count} events from {stream}")
```
