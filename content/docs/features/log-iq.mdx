...
- `X-P-Extract-Log` - Specifies which field in the incoming JSON contains the unstructured log, this has to be at the root level in the ingested event. The field name is usually `log`.
...
If only `X-P-Log-Source` is present then we assume your data is structured. If log format is not detected then we add `p_format_verified=false` to the event. Data is still ingested.
