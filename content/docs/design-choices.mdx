...
#### Staged writes
Staging data locally on the ingestor node for at least a minute, leads to a minor lag in querying the data. We enable immediate persistence for low latency ingestion.
