...
Parseable stores spans as columnar events, so both patterns run on the same data — trace lookup by ID, and the aggregations behind its APM features. Traces are [ingested over OTLP](/ingest-data/otel/traces).
