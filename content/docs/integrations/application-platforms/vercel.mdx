...
The overview shows event totals for each signal and the current log error rate.

![Vercel Integration Observability dashboard overview](./static/vercel/Overview.png)

### Logs

Inspect recent logs, error events, volume by level and requests by HTTP status code.

![Vercel logs dashboard](./static/vercel/Logs.png)

### Traces

Review recent spans, slow spans, average duration and route groups.

![Vercel traces dashboard](./static/vercel/Traces.png)

### Speed Insights

Track LCP, CLS and INP ratings by route.

![Vercel Speed Insights dashboard](./static/vercel/SpeedInsights.png)

### Web Analytics

Analyze recent pageviews by path, event type and origin.
