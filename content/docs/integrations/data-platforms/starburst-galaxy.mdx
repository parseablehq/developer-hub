...
On the Parseable side, `remote_write` sends Prometheus metrics into Parseable. `X-P-Stream: starburst-metrics` decides the destination dataset, and Parseable treats the payload as metrics because it arrives on the metrics endpoint.
