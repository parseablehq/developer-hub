...
app.post('/webhook', async (req, res) => {
  ...
  const opsgenieAlert = {
    ...
    alias: `parseable-${alert.name}-${alert.stream}`,
    description: alert.message || `Alert triggered on stream ${alert.stream}`,
    priority: mapPriority(alert.severity),
    source: 'Parseable',
    tags: ['parseable', alert.stream],
    details: {
      stream: alert.stream,
      query: alert.query,
...
2. **Add Tags** - Include stream and severity tags for filtering
